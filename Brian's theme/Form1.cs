...
public partial class Form1 : Form
{
    ...
    private int _postPatternPauseMs = 1600;
    private bool _followSystemTheme = true;
    private Color _lightBg = Color.White;
    ...
    public Form1()
    {
        ...
        var prefsItem = new ToolStripMenuItem("Preferences...", null, PreferencesMenu_Click);
        var exitItem = new ToolStripMenuItem("Exit", null, ExitMenu_Click);
        settingsMenu.DropDownItems.Add(prefsItem);
        settingsMenu.DropDownItems.Add(exitItem);

        var helpMenu = new ToolStripMenuItem("Help");
        var aboutItem = new ToolStripMenuItem("About", null, AboutMenu_Click);
    ...
    private void PreferencesMenu_Click(object? sender, EventArgs e)
    {
        ...
        {
            _segmentDelayMs = dlg.SegmentDelayMs;
            _postPatternPauseMs = dlg.PostPatternPauseMs;
            _followSystemTheme = dlg.FollowSystemTheme;
            _lightBg = dlg.LightBg;
            _lightFg = dlg.LightFg;
            _darkBg = dlg.DarkBg;
            _darkFg = dlg.DarkFg;
            _isLightTheme = dlg.IsLightTheme;

            // apply the chosen theme (respect follow flag)
            if (_followSystemTheme)
                ApplyTheme(GetIsLightTheme());
            else
                ApplyTheme(_isLightTheme);
    ...
    private void ApplyTheme(bool isLight)
    {
        ...
        // choose colors from configured palettes
        if (_isLightTheme)
        {
            _bgColor = _lightBg;
            _fgColor = _lightFg;
        ...
        {
            _statusLabel.BackColor = _isLightTheme ? SystemColors.Control : Color.FromArgb(30, 30, 30);
    ...
    private async Task AnimationLoopAsync(CancellationToken token)
    {
        try
        {
            while (!token.IsCancellationRequested)
            {
                ...
                // capture current delays so user changes apply to next pattern
                int segmentDelay = _segmentDelayMs;
                int postPause = _postPatternPauseMs;
                ...
                for (int x = 0; x <= 278; x += step)
                    {
                        for (int s = 0; s <= 1; s++)
                        {
                            ...
                            await Task.Delay(segmentDelay, token).ConfigureAwait(false);
                ...
                try { await Task.Delay(postPause, token).ConfigureAwait(false); } catch (OperationCanceledException) { }
            }
        }
        catch (OperationCanceledException) { }
        finally
